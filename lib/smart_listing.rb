...
module SmartListing
  class Base
    ...
    def setup params, cookies
      ...
      @page = get_param :page
      byebug
      @per_page = !get_param(:per_page) || get_param(:per_page).empty? ? (@options[:memorize_per_page] && !get_param(:per_page, cookies).nil? && get_param(:per_page, cookies).gsub(/\s+/, '') != '' && get_param(:per_page, cookies).to_i >= 0 ? get_param(:per_page, cookies).to_i : page_sizes.first) : get_param(:per_page).to_i
